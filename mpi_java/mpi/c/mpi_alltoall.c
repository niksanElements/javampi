    ...
    jlong comm)
{
    ...
    if(sendbuff != NULL){
        if(_sendtype->isBufferType == JNI_FALSE){
            ...
            // allocate _buff
            _sendbuff = malloc(_sendtype->size*sendcount*commSize);
            if(_sendbuff == NULL){
...
#endif
            // copy the buffer
            _sendtype->getBuffAddr(env,sendbuff,sendoffset,sendcount*commSize,&_sendbuff,_sendtype->size,0);
        }
        else{
    ...
    result = MPI_Alltoall(_sendbuff,sendcount,_sendtype->mpi_type,
        _recvbuff,recvcount,_recvtype->mpi_type,(MPI_Comm)comm);
    ...
    if(sendbuff != NULL){
         if(_sendtype->isBufferType == JNI_FALSE)
            ...
            free(_sendbuff);
#elif defined JNI_CRITICAL_SECTION
            _sendtype->relBuffAddr(env,sendbuff,sendoffset,sendcount*commSize,
                &_sendbuff,_sendtype->size,JNI_ABORT);
#endif
    }
