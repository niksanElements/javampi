...
public class ISend_co{
    ...
    public static void main(String[] args) {
    	...
    	if(rank == 0) {
	        ...
	        pArr[0] = p;
            Request req = MPI.COMM_WORLD.isend(pArr,0,1,person,1,0,buff);
            req.Wait();
    	}
    	else if(rank == 1) {
            Person[] pArr = new Person[2];
            Request req = MPI.COMM_WORLD.irecv(buff,0,1,person,0,0);
            Status stat = req.wait(pArr,0,1,person,buff);
